...
* `qupath_xpra.def`: Latest QuPath (0.6.x) with [Xpra](https://xpra.org/) for remote GUI access via browser. Note that extensions can now be installed from within QuPath, so this container does not include any extensions.

#### Fiji
* `fiji_latest.def`: Latest Fiji, suitable for headless use
* `fiji_xpra.def`: Latest Fiji with [Xpra](https://xpra.org/) for remote GUI access via browser.

#### CellProfiler

We recommend using the official released [CellProfiler container from docker-hub](https://hub.docker.com/r/cellprofiler/cellprofiler). Here we provide definitions for CellProfiler v4.2.6, with either StarDist or Cellpose pre-installed, best suited for headless use.

#### Other containers

* If you are looking for a JupyterLab container, we recommend choosing [one of the official Jupyter containers](https://jupyter-docker-stacks.readthedocs.io/en/latest/using/selecting.html). 
* For JupyterLab with virtual desktop integration for GUI display, we recommend using the [official containers of `jupyter-remote-desktop-proxy`](https://quay.io/repository/jupyterhub/jupyter-remote-desktop-proxy).
* For `napari`, we recommend [the official `napari` containers](https://github.com/orgs/napari/packages?repo_name=napari). The `-xpra` one facilitates remote GUI access via the browser.
* Legacy containers can be found in the `legacy_definitions` directory, but these are no longer maintained and may not work with recent versions of software.

## Building a container at JAX
...
## Building a container in the cloud
...
Next, below the code editor, give your container a name, for example for the `qupath_latest.def` one may use: `imageanalysis/qupath:0.6.0`  
...
```bash
singularity pull qupath_0_6_0.sif library://<your Sylabs username>/imageanalysis/qupath:0.6.0
```  
...
```bash
singularity build --remote qupath_0_6_0.sif qupath_latest.def
```
